...
if (file_exists("CRF.rds")) {
  system.time(CRF <- readRDScache("CRF.rds"))
} else {
  file <- paste0(path2data, CRF_file)
  CRF <- file |>
    excel_sheets() |> 
...
A function that generates the presence/absence data (coded as 2/1 here) from the output
...
if (file_exists("K_pneumoniae_raw.rds")) {
  K_pneumoniae_raw <- readRDScache("K_pneumoniae_raw.rds")
} else {
  K_pneumoniae_raw <- presence_abscence(MALDITOF, USUBJID, SampleSchedule,
                                        Identification_MALDITOF,
                                        "Klebsiella pneumoniae")
  saveRDScache(K_pneumoniae_raw, "K_pneumoniae_raw.rds")
}
```
...
if (file_exists("Kp_msm.rds")) {
  Kp_msm <- readRDScache("Kp_msm.rds")
} else {
  Kp_msm <- msm_data(MALDITOF, dates, USUBJID, SampleSchedule, Identification_MALDITOF,
                     "Klebsiella pneumoniae", ADMISSION, DISCHARGE)
  saveRDScache(Kp_msm, "Kp_msm.rds")
}
```
...
if (file_exists("q_array.rds")) {
  q_array <- readRDScache("q_array.rds")
} else {
  q_array <- boot_msm(Kp_msm_fit, qmatrix.msm, 1000, cores = nb_cores)
  saveRDScache(q_array, "q_array.rds")
}
```
...
if (file_exists("pipeline1_results.rds")) {
  pipeline1_results <- readRDScache("pipeline1_results.rds")
} else {
  pipeline1_results <- bacteria_sets |>
    set_names() |> 
    map(pipeline1)
  saveRDScache(pipeline1_results, "pipeline1_results.rds")
}
```
...
if (file_exists("pipeline2_results.rds")) {
  pipeline2_results <- readRDScache("pipeline2_results.rds")
} else {
  pipeline2_results <- bacteria_sets |>
    set_names() |> 
    map(pipeline2)
  saveRDScache(pipeline2_results, "pipeline2_results.rds")
}
```
...
if (file_exists("pipeline3_results.rds")) {
  pipeline3_results <- readRDScache("pipeline3_results.rds")
} else {
  pipeline3_results <- bacteria_sets |>
    set_names() |> 
    map(pipeline3)
  saveRDScache(pipeline3_results, "pipeline3_results.rds")
}
```
...
if (file_exists("pipeline4_results.rds")) {
  pipeline4_results <- readRDScache("pipeline4_results.rds")
} else {
  pipeline4_results <- bacteria_sets |>
    set_names() |> 
    map(pipeline4)
  saveRDScache(pipeline4_results, "pipeline4_results.rds")
}
```
...
if (file_exists("pipeline5_results.rds")) {
  pipeline5_results <- readRDScache("pipeline5_results.rds")
} else {
  pipeline5_results <- bacteria_sets |>
    set_names() |> 
    map(pipeline5)
  saveRDScache(pipeline5_results, "pipeline5_results.rds")
}
```
