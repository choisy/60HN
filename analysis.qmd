...
```{r duration_x_nb_patients}
tmp <- ward_dates |> 
  mutate(duration = as.numeric(DISCHARGE - ADMISSION) / mpd) |> 
  left_join(nb_wards, "ward") |> 
  select(n, duration)

aaa <- gam(duration ~ s(n), Gamma, tmp, method = "REML") |> 
  fitted_values(tibble(n = seq(min(tmp$n), max(tmp$n), le = 512)))
  
with(tmp, plot(n, duration, type = "n",
               xlab = "number of patients enrolled", ylab = "duration of stay (days)"))
with(aaa, {
  polygon2(n, .lower_ci, .upper_ci, col = "lightgrey", border = NA)
  lines(n, .fitted)
})
with(tmp, points(n, duration))
```
