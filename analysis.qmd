...
```{r}
bsm <- function(...) msm(..., qmatrix = matrix(c(0:1, 1:0), 2))
```
...
bsm <- function(formula, subject, data, ...) {
  args <- c(list(formula = formula,
                 ...
                 data    = data,
                 qmatrix = matrix(c(0:1, 1:0), 2)),
            list(...))
  do.call(msm::msm, args)
}
...
bsm(state ~ days, USUBJID, Kp_msm)

Q
aaa <- msm(state ~ days, USUBJID, Kp_msm, qmatrix = Q)
