...
```{r}
estimations <- function(x) {
  out <- cbind(matrix(x$estimates.t, 2), x$ci)
  rownames(out) <- c("colonization", "decolonization")
...
alluvial2 <- function(x, print = TRUE, ...) {
  alluvial(x[, 1:2], freq = x[[3]], col = "steelblue", border = NA, alpha = .8, ...)
  if (print) return(x)
}
```
...
with(the_model, {
  polygon2(n, .lower_ci, .upper_ci, col = adjustcolor(4, .3))
  lines(n, .fitted, col = 4)
})
```
...
```{r}
(prevalences(MALDITOF, nb_positive_samples, nb_samples))
```
...
```{r}
prev_adm_disch <- prevalences_admission |> 
...
The following function is a wrapper around the above function and the the `alluvial2()`
...
Let's try it out on *K. pneumoniae*:

```{r}
pipeline1("Klebsiella pneumoniae")
```

Let's try it out on *E. coli*:

```{r}
pipeline1("Escherichia coli")
```

Let's try it out on *E. hormaechei*

```{r}
pipeline1("Enterobacter hormaechei")
```
