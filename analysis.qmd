...
```{r}
nb_cores <- 11 # (maybe not useful anymore)
```
...
Reading the data (it takes 15" in parallel):
...
#  setNames(sheets) |> 
#  future_map(~ .x |>  # de-duplication of records
#          group_by(USUBJID) |> 
#          group_modify(~ .x |>
#                           arrange(desc(entry)) |> 
#                           first()) |> 
...
CRF <- sheets |> 
  ...
  future_map(~ .x |>  # de-duplication of records
          group_by(USUBJID) |> 
          group_modify(~ .x |>     #TO DO: group_split() |> future_amp() |> bind_rows()
                           arrange(desc(entry)) |> 
                           first()) |> 
          ungroup())
```
