...
A function that computes the data for ward:
...
Computing the ward occupancies:
...
plot_occ <- function(x) {
  x |> 
    ...
    bind_rows(x) |> 
    with({
      plot(date, occupancy, type = "s", xlab = NA)
...
Plotting the wards occupancies:

```{r eval = F}
walk(w_occu, plot_occ)
```
